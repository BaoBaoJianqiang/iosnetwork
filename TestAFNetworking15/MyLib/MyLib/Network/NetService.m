...
-(void)startEngine {
    requestTasksPool = [NSMutableArray new];
    loadingAPICount = 0;
    timeInterval = 0;
}
    ...
    if(config) {
        ...
        if ([strKeyAndValue length] > 0) {
            cacheKey = [[NSString stringWithFormat:@"%@?%@", url, strKeyAndValue]
                            stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];    //编码
        }
        ...
        void (^successBlk)(NSURLSessionDataTask *task, id responseObject)  = ^(NSURLSessionDataTask *task, id responseObject){
            [requestTasksPool removeObject: task];
            ...
            [Globals getInstance].cookie = [allHeaders objectForKey:@"Set-Cookie"];
        ...
        if([netType isEqualToString:@"get"]) {
            ...
            //[Globals getInstance].cookie
            [session.requestSerializer setValue: @"baobao=jianqiang" forHTTPHeaderField:@"Set-Cookie"];
            NSURLSessionDataTask* task = [session GET: url parameters:params success:successBlk failure:failBlk];
            
            [requestTasksPool addObject: task];
        } else {
            if(++loadingAPICount > 0) {
                [viewController showLoading];
            }

            NSURLSessionDataTask* task = [session POST: url parameters:params success:successBlk failure:failBlk];
            [requestTasksPool addObject: task];
        }
    }
}
...
- (void)cancelAllRequest {
    @synchronized(self) {
        [requestTasksPool enumerateObjectsUsingBlock:^(NSURLSessionDataTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
