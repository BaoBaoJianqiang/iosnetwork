...
- (void)initViews {
    ...
    showtimeButton = (UIButton*)[self.view viewWithTag: 3001];
    [showtimeButton addTarget: self action: @selector(showtime) forControlEvents:UIControlEventTouchUpInside];
...
-(void)showtime {
    NSDate* serverTime = [NSDate dateWithTimeIntervalSinceNow: [Globals getInstance].timeInterval];

    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString *strDate = [dateFormatter stringFromDate: serverTime];
    NSLog(strDate);
}
