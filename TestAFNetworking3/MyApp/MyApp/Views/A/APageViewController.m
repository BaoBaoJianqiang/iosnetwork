...
- (void)initViews {
    ...
    label = (UILabel*)[self.view viewWithTag: 230];
...
- (void) visitNetwork {
    ...
    [session GET:@"http://127.0.0.1:8888/cinema/getcinemadetail" parameters:params success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        NSDictionary* str = [dic objectForKey:@"result"];
        
//        Cinema *cinema = [Cinema yy_modelWithJSON:str];
        
        id result = [ObjectMappingLoader loadObjectWithClassName: @"NewCinema" andData:str];
        NewCinema* cinema = (NewCinema*)result;
        [label setText: cinema.cinemaName];
